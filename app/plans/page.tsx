...
import { clinicalPillars, planCountByPillar, planCountByStatus, sampleCarePlans, updatePlanStatus, type CarePlan, type ClinicalPillar, type PlanStatus } from '../../lib/care-plans';
...
export default function CarePlansPage() {
  ...
  const [pillarFilter, setPillarFilter] = useState<ClinicalPillar | 'All pillars'>('All pillars');
  const [message, setMessage] = useState('');
  const counts = useMemo(() => planCountByStatus(plans), [plans]);
  const pillarCounts = useMemo(() => planCountByPillar(plans), [plans]);
  const visible = plans.filter((plan) => (filter === 'All' || plan.status === filter) && (pillarFilter === 'All pillars' || plan.pillar === pillarFilter));
  ...
  return <main className="shell" style={{ minHeight: '100vh', display: 'block', padding: 'clamp(1.25rem, 4vw, 4rem)', maxWidth: 1120, margin: '0 auto' }}>
    ...
    <div className="page-heading" style={{ marginTop: '2rem' }}>
      <div><span className="eyebrow">CARE PLAN REVIEW</span><h1>Plans aligned to CareGrid’s four clinical pillars.</h1><p>Fictional care-plan summaries for diabetes, hypertension, cardiovascular health and stroke prevention. Changes stay only in this page until it is refreshed.</p></div>
    ...
    <section className="panel" style={{ marginTop: '1.5rem' }}>
      <div className="panel-heading"><div><span className="eyebrow">CLINICAL FOUNDATION</span><h2>Four connected-care pillars</h2></div><span className="muted">Strategic pathways · not diagnostic categories</span></div>
      <div className="device-grid" style={{ marginTop: '1rem' }}>
        {clinicalPillars.map((pillar) => <button key={pillar.name} className="panel" style={{ textAlign: 'left', cursor: 'pointer' }} aria-pressed={pillarFilter === pillar.name} onClick={() => setPillarFilter(pillarFilter === pillar.name ? 'All pillars' : pillar.name)}>
          <span className="eyebrow">{String(pillarCounts[pillar.name]).padStart(2, '0')} SAMPLE PLANS</span>
          <h3>{pillar.name}</h3>
          <p className="muted">{pillar.description}</p>
        </button>)}
      </div>
    </section>
    <div className="tabs" aria-label="Filter care plans by status" style={{ margin: '2rem 0 1rem' }}>
      {(['All', ...statuses] as const).map((status) => <button key={status} aria-pressed={filter === status} onClick={() => setFilter(status)}>{status}</button>)}
    </div>
    <div className="tabs" aria-label="Filter care plans by clinical pillar" style={{ margin: '0 0 1rem' }}>
      {(['All pillars', ...clinicalPillars.map((pillar) => pillar.name)] as const).map((pillar) => <button key={pillar} aria-pressed={pillarFilter === pillar} onClick={() => setPillarFilter(pillar)}>{pillar}</button>)}
    </div>
    <p className={message ? 'status' : 'sr-only'} role="status">{message}</p>
    {visible.length === 0 ? <section className="panel empty"><h2>No sample plans in this pathway yet.</h2><p>This pillar is part of the CareGrid product direction but is not represented by a fictional plan in the current demo dataset.</p><button className="secondary" onClick={() => { setFilter('All'); setPillarFilter('All pillars'); }}>Show all plans</button></section> : <div className="device-grid">
      {visible.map((plan) => {
        ...
        return <article className="panel" key={plan.id}>
          ...
          <span className="badge" style={{ marginTop: '0.75rem' }}>{plan.pillar}</span>
